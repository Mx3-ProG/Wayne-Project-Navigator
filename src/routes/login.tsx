...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in to your Wayne Client Portal with a one-time code sent by email.",
      },
    ],
  }),
  component: LoginPage,
...
function LoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      await requestLoginCode({ data: { email: normalized } });
    ...
    <div className="flex min-h-screen items-center justify-center px-5 py-14">
      ...
      >
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-5 font-display text-3xl font-semibold">{t("auth.login.title")}</h1>
        ...
        <GlassCard variant="strong" interactive={false} className="p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" size="lg" className="w-full" disabled={loading}>
              {loading ? t("auth.submit.loading") : t("auth.login.submit")}
            </Button>
          </form>
